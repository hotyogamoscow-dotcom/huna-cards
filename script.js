...
const theoryData = [
	{
    	...
    	title: "1",
    	sub: ``,
    	content: `Алакаи (помощник) должен был получить посвящение на высшие уровни, и он знал, что это будет особое посвящение. Пять кахун, три женщины и два мужчины, расположились вокруг него, пока он лежал на твердом блоке лавы, с которым он уже был довольно хорошо знаком, поэтому он чувствовал себя очень комфортно. Кахуна По (старший кахуна) произнес, объяснил посвящение и закончил словами: «Это символы Ули, и как посвященный, помни, что, поскольку эти символы призывают энергии очень высокого уровня, их следует использовать во благо всех заинтересованных лиц. Использование их или разрешение использовать их для чего-либо, кроме добрых дел, контролируется покровительствующей богиней символов, самой богиней Улиули. Она будет настаивать на их использовании. Позаботьтесь о том, чтобы предостеречь своих посвященных.
...
Далее следует ещё одно описание сложной системы символизации мыслей, которая, как мы также считаем, существовала до прихода западных людей на Гавайи. По крайней мере, это символизировало мысли... а возможно, и нечто большее!`
	},
	{
    	...
    	title: "3. Символы",
    	sub: ``,
    	content: `Na Ao Opua Kilokilo, Ka Haiki A Na Lani — это набор из 36 символов, обнаруженных в серии рукописей, принадлежащих Кахуне Куаухаоалити. Рукописи датируются приблизительно 1860 годом. Рукописи попали в руки Киайна'ауаомаикалани (Тэда Джеймса) в сентябре 1990 года. На вопрос дядя Джордж (Ланакилакеикиахиали'и) ответил: «Я не уверен, кто такой Куаухаоали'и, но помню, как тетя Ло (Иолани Луахине) говорила мне, что он был тяжелым!» Рукописи были скопированы для сохранения.`
	},
	{
    	id: 4,
    	title: "4. Для предсказаний",
    	sub: ``,
    	content: `На первый взгляд, символы на рукописях выглядят так, будто они предназначены для Килокило, или предсказаний. Кахуны Гавайев в прежние времена были экспертами в предсказаниях, и одним из способов было чтение Опуа, или облаков. На самом деле, поверхностное значение Опуа скрывает истинный смысл. Давайте рассмотрим более глубокий смысл Опуа и его символов:
...
А На Лани: С небес.`
	},
	{
    	...
    	sub: ``,
    	content: `Таким образом, «На Ао Опуа Килокило Хайки А На Лани» может быть процессом, посредством которого человек переживает просветленные состояния в результате воздействия богов или энергий категории Нохо. Средство Нохо — это нисхождение света с небес через высшую Ману, пребывающую на дыхании.
...
Рассматривая названия символов 1, 2 и 3, мы обнаруживаем, что имя Ули присутствует во всех трёх. `
	},
	{
    	...
    	title: "8. Контролёр",
    	sub: ``,
    	content: `Ули (или Улиули) — это женский (божественный) аспект Кане. В первом творении И'о (непроявленный источник всего творения, см. главу 11) сотворил Кане-творца. Женский аспект Кане — На' Вахине. Тайное имя Кане (тайное потому, что знание истинного имени бога или богини позволяет призывать этого бога) — Элиели. Тайное имя На Вахине — Улиули, или просто Ули.
...
Таким образом, Ули, управляющая божественная сущность этих символов, является высшим женским аспектом творения. Поскольку Мана находится в сфере женского начала, особенно уместно, что контроль над символами находится в руках Ули. Человек без энергии богини немотивирован — не стремится к действию. Женщина — творец, именно она создает жизнь и, следовательно, пробуждает жизненную силу — Ману.`
	},
	{
    	...
    	sub: ``,
    	content: `Каждый следующий символ призывает бога, богиню или ангела (каждый из них — аспект Ули), которые войдут и Нохо (проникнут внутрь) в пользователя. Для максимальной эффективности символы необходимо использовать вместе с именем, которое на них изображено. Существует три уровня использования символов:
...
В эту систему заложен огромный экологический аспект, поскольку для установления связи с невидимыми уровнями требуется инициация. Это означает, что непосвященные не смогут использовать символы ни для чего, кроме личного пользования, поэтому инициация необходима для установления связи с невидимыми уровнями. Полная инициация необходима для наделения силой на самом высоком уровне.`
	},
	{
    	...
    	title: "10. Медитация",
    	sub: ``,
    	content: `Каждый из следующих символов представляет бога, богиню или ангела и вызывает соответствующий Нохо. Чтобы использовать их в медитации, думайте о символе целиком, о цвете, названии, звуке и воспоминании о том, что вы чувствовали, когда получили этот символ при инициации. Теперь ждите Нохо. Вы почувствуете, как он приходит сверху, возможно, через Манава (корону) или через Хону (отверстие в затылке у основания черепа, где позвоночник входит в череп и где начинается Нохо).
...
Один из способов понять, что следующие символы подобны «янтрам» (восточно-индийским диаграммам), которые при медитации вызывают определенное состояние сознания.`
	},
	{
    	...
    	title: "11. Философия",
    	sub: ``,
    	content: `Поскольку всё существующее может быть символизировано, приведённые выше символы являются лишь символами функции или энергии Вселенной. Знание истинного и полного имени и символа явления или сущности, согласно верованиям Хуна, даёт вам контроль над ним. На Гавайях имя отражает сущность, вибрацию и призывает или вызывает то, что названо.
